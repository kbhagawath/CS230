...
Line *make_lines(int line_count, int block_size) {
  ...
  Line *line = (Line *) malloc(line_count*sizeof(Line));
  for ( int i = 0; i < line_count; i ++){
    struct Line temp = {.valid = 0, .tag = 0, .accessed = make_block(block_size), .block_size = block_size};
    line[i] = temp;
  }
  return line;
}
...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  for (int i = 0; i < set_count; i++){
    struct Set temp = {.lines = make_lines(line_count, block_size), .line_count = line_count, .lru_queue = NULL};
    set[i] = temp;
  }
  return set;
}
...
Cache *make_cache(int set_bits, int line_count, int block_bits) {
  ...
  struct Cache temp = { .sets = make_sets(set_count, line_count, block_count), .set_count = set_count, .line_count = line_count, .block_size = block_count, .set_bits = set_bits, .block_bits = block_bits};
  cache[0] = temp;
  // END TODO
